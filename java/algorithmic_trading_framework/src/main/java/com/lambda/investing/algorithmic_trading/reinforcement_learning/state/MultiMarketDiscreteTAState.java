...
public class MultiMarketDiscreteTAState extends DiscreteTAState {
	...
	public static DiscreteTAState create(Instrument instrument, List<Instrument> otherInstruments,
		...
		if (instrument != null && instrument.isFX()) {
			System.out.println(Configuration.formatLog("{} MultiMarketDiscreteTAState detected as FX -> MultiMarketDiscreteTAStateFX", instrument.getPrimaryKey()));
			return new MultiMarketDiscreteTAStateFX(instrument, otherInstruments, scoreEnumColumn, candleType, periods, numberOfDecimals, marketHorizonSave, periodsMsZscore);
		} else {
//			System.out.println(Configuration.formatLog("{} MultiMarketDiscreteTAState detected as not FX",instrument.getPrimaryKey()));
			return new MultiMarketDiscreteTAState(instrument, otherInstruments, scoreEnumColumn, candleType, periods, numberOfDecimals, marketHorizonSave, periodsMsZscore);
		}

	}

	protected MultiMarketDiscreteTAState(Instrument instrument, List<Instrument> otherInstruments,
										 ScoreEnum scoreEnumColumn, CandleType candleType, int[] periods, int numberOfDecimals,
										 int marketHorizonSave, int[] periodsMsZscore) {
	...
	public synchronized void reset() {
		logger.info("resetting MultiMarketDiscreteTAState");
