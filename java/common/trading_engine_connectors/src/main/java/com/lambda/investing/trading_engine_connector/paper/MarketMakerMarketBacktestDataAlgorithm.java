...
public class MarketMakerMarketBacktestDataAlgorithm implements MarketDataListener, ExecutionReportListener {
    ...
    public MarketMakerMarketBacktestDataAlgorithm(PaperTradingEngine tradingEngineConnector) {
    ...
    private void waitToBeFree() {
        ...
        while (tradingEngineConnector.isBusy()) {
            //wait to be free
            if (!isBusy) {
                isBusy = true;
                logger.warn("waiting to be free tradingEngineConnector: " + tradingEngineConnector.toString());
            }
