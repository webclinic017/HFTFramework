...
class DarwinexFtpDownloader:
    ...
    def _save_parquet(self, symbol, day_to_add, df):
        ...
        try:
            df.to_parquet(
                output_path + 'data.parquet', compression='GZIP', engine='fastparquet', use_dictionary=False
            )
        except:
            df.to_parquet(output_path + 'data.parquet', compression='GZIP', use_dictionary=False)
...
if __name__ == '__main__':
    darwinex_ftp_downloader = DarwinexFtpDownloader(
        username='tradEASystems', password='vL4hz9z77MZ5px'
    )
