...
class TimeCandlePublisher(BaseCandlePublisher):
    class InstrumentCandle:
        ...
        def is_new_candle_elapsed(self, date_to_check: datetime.datetime) -> bool:
            ...
            if self.start_of_day is None or date_to_check.day > self.start_of_day.day:
                print(f"new day detected {date_to_check} => calculating candle times")
                self.start_of_day = date_to_check.replace(hour=0, minute=0, second=0)
                self.candle_times = [self.start_of_day + datetime.timedelta(seconds=i) for i in
                                     range(0, 24 * 60 * 60, self.seconds_threshold)]
                self.candle_times_index = self._find_candle_index(date_to_check)

            return date_to_check >= self.candle_times[self.candle_times_index]

        def _find_candle_index(self, date_to_check: datetime.datetime) -> int:
            '''
...
if __name__ == "__main__":
    ...
    kraken.marketdata.port=6630
    kraken.tradeengine.port=6631
